...
def read_data_pakistan(basedir):
    ...
    dsall = xr.open_mfdataset(basedir+'results WAM2layers/backtrack_*T00-00.nc', combine = 'nested', concat_dim='time').rename(latitude='lat', longitude='lon')
    ...
    # E5: 
    temp = xr.open_dataset(basedir+"results UGhent HAMSTER/Pakistan simulations/bias_corrected/ens5_rhadap80/bias_corrected_20220810120000_rhadap80.nc")
    srcs_ghent_e5 = temp["E2P_BC"].mean("time")

    for date in range(11,25):
        temp = xr.open_dataset(basedir+"results UGhent HAMSTER/Pakistan simulations/bias_corrected/ens5_rhadap80/bias_corrected_202208" + str(date) + "120000_rhadap80.nc")
        srcs_ghent_e5 += temp["E2P_BC"].mean("time")
    ...
    #convert to -180 to 180 lon
    ds_flexpart_tatfancheng.coords['lon'] = (ds_flexpart_tatfancheng.coords['lon'] + 180) % 360 - 180
    ...
    ## Flexpart Ru Xu                                     ##
    ########################################################
    ds_flexpart_xu = xr.open_dataset(basedir+"results Ru_Xu_FLEXPART/e_daily.nc").rename(latitude='lat', longitude='lon')
    srcs_flexpart_xu = ds_flexpart_xu["variable"].sum("time")

    ########################################################
    ## Lagranto CHc                                       ##
    ########################################################
    ds_lagranto_CHc = xr.open_dataset(basedir+"results CHc LAGRANTO/Pakistan_2022_CHc_eventtotal_ens1.nc").rename(dimx_N='lon', dimy_N='lat')
    ...
    ## 2LDRM                                              ##
    ########################################################
    ds_2ldrm = xr.open_dataset(basedir+"results 2LDRM/2LDRM_Pakistan_case_gl.nc").rename(latitude='lat', longitude='lon')
    srcs_2ldrm = ds_2ldrm["moisture_source"].sum("time").T

    ########################################################
    ## FLEXPART UiB                                       ##
    ########################################################
    ds_flexpart_uib = xr.open_dataset(basedir+"results UiB FLEXPART WaterSip/Pakistan_2022_UiB_Sodemann_grid_EN1_regridded.nc")
