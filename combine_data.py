...
from Functions import get_grid_info_new
...
def read_btrims(basedir, casename):
    ...
    # Set boundary moisture sources to 0
    if casename == "Australia":
        ds[:, ds["longitude"] == -151.5] = 0
...
def read_precip_era5(basedir, casename, exclude=[]):
    ...
    for model in [
        ...
        if model == "results 2LDRM":
            ...
            variable = "precip_estimate"  # This is not ERA5
            ...
            if casename == "Australia":
                filename = "/PR_ERA5_TRACMASS_22-28feb2022.nc"
            elif casename == "Pakistan":
                filename = "/PR_ERA5_TRACMASS.nc"
            elif casename == "Scotland":
                filename = "/PR_ERA5_TRACMASS_new_06-08oct2023.nc"
            variable = "PR_TRACMASS"
            name = "TRACMASS"
            ...
            filename = "/" + casename + "_precip.nc"
            variable = "precip_era5"
            if model == "results UGhent HAMSTER":
        ...
        if (
        ...
        ):  # no precipitation timeline available (or not as mm over sink region)
            if model == "results CHc LAGRANTO":
                ...
                if casename == "Pakistan":
                    a_gridcell_newp, l_ew_gridcellp, l_mid_gridcellp = (
                        get_grid_info_new(
                            np.arange(24, 30.1, 0.25), np.arange(67, 71.1, 0.25)
                        )
                    )
                    A[name] = (
                        ds["precip_era5"][:, 240:265, 988:1005] * a_gridcell_newp
                    ) / (a_gridcell_newp.sum() * 18)
                elif casename == "Scotland":
                    a_gridcell_newp, l_ew_gridcellp, l_mid_gridcellp = (
                        get_grid_info_new(
                    ...
                    A[name] = (
                        ds["precip_era5"][:, 120:153, 688:717] * a_gridcell_newp
                    ) / (a_gridcell_newp.sum() * 29)
                elif casename == "Australia":
                    a_gridcell_newp, l_ew_gridcellp, l_mid_gridcellp = (
                        get_grid_info_new(
                    ...
                    A[name] = (
                        ds["precip_era5"][:, 448:489, 1316:1353] * a_gridcell_newp
                    ) / (a_gridcell_newp.sum() * 37)
                A[name] = A[name].sum(["lon", "lat"])
            elif model == "results WAM2layers":
                ds = xr.open_mfdataset(
                    path + filename, combine="nested", concat_dim="time"
                )  # .sum("time")
                a_gridcell_new, l_ew_gridcell, l_mid_gridcell = get_grid_info_new(
                    np.arange(-80, 80.1, 0.25), np.arange(-180, 180, 0.25)
                )
                if casename == "Pakistan":
                    a_gridcell_newp, l_ew_gridcellp, l_mid_gridcellp = (
                        get_grid_info_new(
                    ...
                    A[name] = (ds[variable] * a_gridcell_new) / (
                        a_gridcell_newp.sum() * 17
                    )
                elif casename == "Scotland":
                    a_gridcell_newp, l_ew_gridcellp, l_mid_gridcellp = (
                        get_grid_info_new(
                ...
                if casename == "Pakistan":
                    ...
                    nrdays = 7
                    nrpoints = 1517
                A[name] = A[name] / nrpoints
            elif model == "results Ru_Xu_FLEXPART":
                ds = xr.open_dataset(path + filename, decode_times=True)
                a_gridcell_new, l_ew_gridcell, l_mid_gridcell = get_grid_info_new(
                    np.arange(-90, 90.1, 0.25), np.arange(-180, 180, 0.25)
                )
                if casename == "Pakistan":
                    a_gridcell_newp, l_ew_gridcellp, l_mid_gridcellp = (
                        get_grid_info_new(
                    ...
                    A[name] = (ds[variable][240:265, 988:1005] * a_gridcell_newp) / (
                        a_gridcell_newp.sum() * 17
                    )
                elif casename == "Scotland":
                    a_gridcell_newp, l_ew_gridcellp, l_mid_gridcellp = (
                    ...
                    A[name] = (ds[variable][120:153, 688:717] * a_gridcell_newp) / (
                        a_gridcell_newp.sum() * 29
                    )
                elif casename == "Australia":
                    a_gridcell_newp, l_ew_gridcellp, l_mid_gridcellp = (
            ...
            if model == "results FLEXPART_WaterSip_TatFanCheng":
                A[name].values = np.full(
                    A[name].values.shape, np.nan
                )  # Set to Nan for now since it is not ERA5 data.
    return A


def read_tracked_precip(basedir, casename, exclude=[]):
    A = {}
            ...
            if model == "results CHc LAGRANTO":
                ...
                if casename == "Scotland":
                    df_Stohl = pd.read_csv(
                        path + "/Total_Lagrangian_precip_SJ05_back.txt",
                        sep=" ",
                        decimal=",",
                        index_col=0,
                    ...
                    df_Stohl = pd.read_csv(
                        path + "/Total_Lagrangian_precip_SJ05.txt",
                        sep=" ",
                        decimal=",",
                        index_col=0,
                        names=["Index", "FLEXPART-Stohl&James"],
